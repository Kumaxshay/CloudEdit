...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const createDirectory = useCallback(
        (
            ...
            let parentDirIdToUse = parentDirId
            if (!parentDirIdToUse) parentDirIdToUse = fileStructure.id

            // Validate parent dir
            const parentDir = findParentDirectory(fileStructure, parentDirIdToUse)
            if (!parentDir) parentDirIdToUse = fileStructure.id

            const addDirectoryToParent = (
                directory: FileSystemItem,
            ): FileSystemItem => {
                if (directory.id === parentDirIdToUse) {
    ...
    const createFile = useCallback(
        (
            ...
            let parentDir = findParentDirectory(fileStructure, parentDirIdToUse)
            if (!parentDir) {
                parentDirIdToUse = fileStructure.id
                parentDir = fileStructure
            }
