...
function Editor() {
    ...
    const getLanguageFromFileName = (fileName: string): string => {
        ...
        return extMap[ext] || language.toLowerCase()
    }
    ...
    useEffect(() => {
        ...
        const detectedLang = activeFile?.name
            ? getLanguageFromFileName(activeFile.name)
            : language.toLowerCase()

        const langExt = loadLanguage(detectedLang as LanguageName)
        if (langExt) {
            extensions.push(langExt)
        }

        setExtensions(extensions)
    }, [filteredUsers, language, activeFile?.name, handleSelectionChange])
